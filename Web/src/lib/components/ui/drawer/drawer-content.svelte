...
<DrawerPrimitive.Portal {...portalProps}>
	...
	<DrawerPrimitive.Content
		...
		class={cn(
			"group/drawer-content bg-background/80 backdrop-blur-2xl fixed z-50 flex h-auto flex-col border-glass-border shadow-[0_-8px_32px_-8px_var(--glass-shadow),inset_0_1px_1px_var(--glass-highlight)]",
	...
	>
		<div
			class="bg-muted/60 mx-auto mt-4 hidden h-1.5 w-[60px] shrink-0 rounded-full group-data-[vaul-drawer-direction=bottom]/drawer-content:block"
		></div>
		{@render children?.()}
	</DrawerPrimitive.Content>
