<script lang="ts">
	...
	let { loading, loggedIn }: { loading: boolean; loggedIn: boolean } = $props();
	const id = $props.id();
...
<Card.Root class="w-sm h-120">
	{#if loading}
		<Card.Content class="gap-2 flex h-full w-full flex-col items-center justify-center">
	...
	{:else if loggedIn}
		...
		<Card.Content class="gap-6 flex h-full w-full flex-col items-center justify-center">
			<p class="text-center">
				You are logged in.
				<br />
			...
			<form>
				<FieldGroup>
					...
					<Field>
						...
						<Input id="email-{id}" type="email" placeholder="m@example.com" required disabled />
					</Field>
					<Field>
						<div class="flex items-center">
							<FieldLabel for="password-{id}">Password</FieldLabel>
						</div>
						<Input id="password-{id}" type="password" required disabled />
					</Field>
					<Field>
